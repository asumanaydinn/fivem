const TrackList = () => {
  return (
    <div className="flex flex-col gap-y-2">
      <div className="w-[342px] h-[48.14px] relative">
        <div className="w-[342px] h-[48.14px] left-0 top-0 absolute bg-zinc-300 bg-opacity-5 rounded-[1px]" />
        <div className="w-[96.12px] h-[32.09px] left-[9.03px] top-[8.02px] absolute">
          <div className="w-[32.09px] flex items-center justify-center h-[32.09px] left-0 top-0 absolute bg-stone-500 rounded-[1px]">
            <svg
              width="13"
              height="14"
              viewBox="0 0 13 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M11.7011 1.71409L6.23054 0.0729316C6.14886 0.0484147 6.06259 0.0433315 5.9786 0.0580878C5.89461 0.0728441 5.81523 0.107031 5.7468 0.15792C5.67838 0.208809 5.62279 0.27499 5.58449 0.35118C5.54619 0.42737 5.52623 0.51146 5.52621 0.596736V7.99974C4.96596 7.49864 4.2507 7.20501 3.49996 7.16792C2.74922 7.13083 2.0085 7.35253 1.40159 7.79596C0.794674 8.2394 0.358341 8.8777 0.165513 9.6042C-0.027315 10.3307 0.0350445 11.1014 0.34217 11.7874C0.649296 12.4734 1.18258 13.0333 1.85289 13.3734C2.52321 13.7135 3.28994 13.8132 4.02494 13.6559C4.75995 13.4986 5.41869 13.0938 5.89109 12.5091C6.36348 11.9244 6.62089 11.1954 6.62031 10.4437V4.61416L11.3865 6.04403C11.4682 6.06854 11.5545 6.07363 11.6385 6.05887C11.7225 6.04411 11.8018 6.00993 11.8703 5.95904C11.9387 5.90815 11.9943 5.84197 12.0326 5.76578C12.0709 5.68959 12.0908 5.6055 12.0909 5.52022V2.2379C12.0908 2.12029 12.0529 2.00582 11.9827 1.91147C11.9125 1.81712 11.8137 1.74791 11.7011 1.71409Z"
                fill="#101010"
              />
            </svg>
          </div>
          <div className="w-[54px] h-[23.04px] left-[42.12px] top-[3.01px] absolute">
            <div className="left-0 top-0 absolute text-neutral-200 text-[9.26px] font-semibold font-['Inter']">
              Music Name
            </div>
            <div className="left-0 top-[15.04px] absolute text-neutral-400 text-[6.59px] font-semibold font-['Inter']">
              Song Artist
            </div>
          </div>
        </div>
      </div>
      <div className="w-[342px] h-[48.14px] relative">
        <div className="w-[342px] h-[48.14px] left-0 top-0 absolute bg-zinc-300 bg-opacity-5 rounded-[1px]" />
        <div className="w-[8.19px] h-[8.19px] left-[317.81px] top-[19.89px] absolute">
          <div className="w-[1.78px] h-[9.80px] left-0 top-[1.26px] absolute origin-top-left -rotate-45 bg-neutral-500 rounded-[0.50px]" />
          <div className="w-[1.78px] h-[9.80px] left-[1.26px] top-[8.19px] absolute origin-top-left rotate-[-135deg] bg-neutral-500 rounded-[0.50px]" />
        </div>
        <div className="w-[96.12px] h-[32.09px] left-[9.03px] top-[8.02px] absolute">
          <div className="w-[32.09px] flex items-center justify-center h-[32.09px] left-0 top-0 absolute bg-stone-500 rounded-[1px]">
            <svg
              width="13"
              height="14"
              viewBox="0 0 13 14"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M11.7011 1.71409L6.23054 0.0729316C6.14886 0.0484147 6.06259 0.0433315 5.9786 0.0580878C5.89461 0.0728441 5.81523 0.107031 5.7468 0.15792C5.67838 0.208809 5.62279 0.27499 5.58449 0.35118C5.54619 0.42737 5.52623 0.51146 5.52621 0.596736V7.99974C4.96596 7.49864 4.2507 7.20501 3.49996 7.16792C2.74922 7.13083 2.0085 7.35253 1.40159 7.79596C0.794674 8.2394 0.358341 8.8777 0.165513 9.6042C-0.027315 10.3307 0.0350445 11.1014 0.34217 11.7874C0.649296 12.4734 1.18258 13.0333 1.85289 13.3734C2.52321 13.7135 3.28994 13.8132 4.02494 13.6559C4.75995 13.4986 5.41869 13.0938 5.89109 12.5091C6.36348 11.9244 6.62089 11.1954 6.62031 10.4437V4.61416L11.3865 6.04403C11.4682 6.06854 11.5545 6.07363 11.6385 6.05887C11.7225 6.04411 11.8018 6.00993 11.8703 5.95904C11.9387 5.90815 11.9943 5.84197 12.0326 5.76578C12.0709 5.68959 12.0908 5.6055 12.0909 5.52022V2.2379C12.0908 2.12029 12.0529 2.00582 11.9827 1.91147C11.9125 1.81712 11.8137 1.74791 11.7011 1.71409Z"
                fill="#101010"
              />
            </svg>
          </div>{" "}
          <div className="w-[54px] h-[23.04px] left-[42.12px] top-[3.01px] absolute">
            <div className="left-0 top-0 absolute text-neutral-200 text-[9.26px] font-semibold font-['Inter']">
              Music Name
            </div>
            <div className="left-0 top-[15.04px] absolute text-neutral-400 text-[6.59px] font-semibold font-['Inter']">
              Song Artist
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default TrackList;
